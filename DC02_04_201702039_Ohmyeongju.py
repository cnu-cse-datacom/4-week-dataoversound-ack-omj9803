...
def sound_print(stream, freq, t):
    n = int(t * 44100)
    interval = 2 * np.pi * freq / 44100
...
def listen_linux(frame_rate=44100, interval=0.1):
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            ...
            toAscii = list(bytearray(byte_stream)) # convert byte to bytearray and make list
            print(toAscii)
            ...
            try:
                ...
                byte_stream_div = byte_stream.split(' ')
    
                if byte_stream_div[0] == "201702039":
                    display(byte_stream_div[1])

                    codec = RSCodec(FEC_BYTES).encode(byte_stream_div[1].encode("utf-8"))
                    freqs = []
                    for k in codec:
                        f1 = (int(bin(k),2) & 0b11110000) >> 4
                        freq1 = START_HZ + (STEP_HZ * f1)
                        ...
                        f2 = int(bin(k),2) & 0b1111
                        freq2 = START_HZ + (STEP_HZ * f2)
                        freqs.append(freq2)

                        idx = codec.index(k)
                        print(f1, f2)
                        print("freq:", freqs[idx])
                        print("freq:", freqs[idx+1])

                    freqs.append(HANDSHAKE_END_HZ)
                    print("freq:", HANDSHAKE_END_HZ)
                    print()
                    print()
